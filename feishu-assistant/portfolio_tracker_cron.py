...
async def main():
    ...
    auto_mode = "--auto" in args  # 新增：自动模式，根据持仓市场智能追踪
    force_market = None
    ...
    # 自动模式：获取所有持仓，根据每个持仓的市场决定是否追踪
    if auto_mode:
        print("📢 自动模式：智能追踪持仓（仅追踪开市的市场）")

        # 获取所有持仓
        all_holdings = get_holdings(FEISHU_USER_OPEN_ID)
        ...
        for h in all_holdings:
            market = h.get('market', 'A股')  # 默认A股
            if market not in holdings_by_market:
                holdings_by_market[market] = []
            holdings_by_market[market].append(h)

        print(f"📊 持仓分布: {', '.join([f'{m}({len(h)}只)' for m, h in holdings_by_market.items()])}")

        # 显示市场状态
        status = MarketTradingHours.get_market_status(now)
        print("\n市场状态:")
        for market, status_text in status.items():
            holdings_count = len(holdings_by_market.get(market, []))
            if holdings_count > 0:
                print(f"  {market}: {status_text} (持仓{holdings_count}只)")
            else:
                print(f"  {market}: {status_text} (无持仓)")

        # 只追踪开市且有持仓的市场
        markets_to_track = []
        for market, holdings in holdings_by_market.items():
            if holdings and MarketTradingHours.is_trading_time(market, now):
                markets_to_track.append(market)

        if not markets_to_track:
            print("\n⏸️ 持仓市场均未开市，跳过追踪")
            return 0

        print(f"\n✅ 即将追踪: {', '.join(markets_to_track)}")
